...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({
  ...
  return (
    <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-[#08090D]/95 border-t border-slate-800/90 backdrop-blur-xl px-2 py-2 shadow-[0_-8px_24px_rgba(0,0,0,0.6)] font-mono">
      <div className="flex items-center justify-around gap-1">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center min-w-[52px] min-h-[48px] py-1 px-1.5 rounded-xl transition-all active:scale-90 cursor-pointer ${
                isActive 
        ...
        <button
          ...
          className="flex flex-col items-center justify-center min-w-[52px] min-h-[48px] py-1 px-1.5 rounded-xl bg-cyan-950/40 border border-cyan-500/40 text-cyan-300 transition-all active:scale-90 cursor-pointer shadow-[0_0_10px_rgba(32,227,255,0.2)]"
        >
          <Bot className="w-4 h-4 text-[#20E3FF] animate-pulse" />
          <span className="text-[10px] tracking-tight mt-0.5 leading-tight text-cyan-300 font-bold">Ask AI</span>
        </button>
      </div>
    </div>
  );
